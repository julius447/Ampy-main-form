/* ============================================================================
   Ampy — Main form · interaction layer · FluentSnippets delivery.
   FORMAT ONLY — behaviour is identical to the approved reference (js/app.js):
   Photon address autocomplete (whole of Sweden), forgiving validation, full
   ARIA (activedescendant / invalid / describedby), honeypot, in-place success,
   error state.

   Packaging changes (mechanism only, no UX change):
     • IIFE + "use strict"; multi-instance boot over every .ampy-mf root, with a
       root.dataset.booted guard; EVERY DOM query scoped to that root (no
       document-wide lookups; ids that could repeat across instances → scoped
       name/class selectors; generated ARIA ids namespaced per instance).
     • Submit transport → SAME-ORIGIN WordPress REST endpoint (window.AMPY_MF =
       { endpoint, nonce }) with header X-WP-Nonce + credentials:"same-origin".
       No CORS problem → the PROTOTYPE/CORS-tolerant branch is DROPPED. Success
       ONLY on a real 2xx { ok:true }; any failure/timeout shows the error state
       (never fake success). 12s AbortController timeout kept. trackLead fires
       ONLY on a delivered 2xx.
     • Photon fetch kept as-is (external, fine client-side).
   FluentSnippets placement: JS Snippet · Frontend · wp_footer.
   ============================================================================ */
(function () {
  "use strict";

  var PHOTON = "https://photon.komoot.io/api/";
  var instSeq = 0; // per-page instance counter → namespaces generated ARIA ids

  function init(root) {
    if (root.dataset.booted === "1") return;
    root.dataset.booted = "1";

    var uid = "mf-" + (++instSeq) + "-";   // unique per instance, keeps ARIA ids collision-free

    var form = root.querySelector(".mf-form");
    if (!form) return;

    var els = {
      forenamn:  root.querySelector("[name=forenamn]"),
      efternamn: root.querySelector("[name=efternamn]"),
      email:     root.querySelector("[name=email]"),
      telefon:   root.querySelector("[name=telefon]"),
      addrSok:   root.querySelector("[name=adress_sok]"),
      addrWrap:  null,
      addrList:  root.querySelector(".mf-addr__list"),
      manualBtn: root.querySelector(".mf-addr__manual"),
      manualFields: root.querySelector(".mf-manual-fields"),
      adress:    root.querySelector("[name=adress]"),
      postnummer: root.querySelector("[name=postnummer]"),
      ort:       root.querySelector("[name=ort]"),
      meddelande: root.querySelector("[name=meddelande]"),
      hp:        root.querySelector("[name=company_url]"),
      submit:    root.querySelector(".mf-submit"),
      error:     root.querySelector(".mf-error"),
      success:   root.querySelector(".mf-success")
    };
    ...
    function setActive(n) {
      ...
      els.addrSok.setAttribute("aria-activedescendant", n > -1 ? uid + "addr-opt-" + n : "");
    }
    function renderList(items, state, q) {
      ...
      els.addrList.innerHTML = items.map(function (a, n) {
        return '<button type="button" id="' + uid + 'addr-opt-' + n + '" class="mf-addr__opt" role="option" aria-selected="false" data-i="' + n + '">' +
    ...
    /* ---- consent-gated tracking (fail-CLOSED: no consent tool → no events) -- */
    var started = false;
    function consentOk() {
    ...
    function collect() {
      return {
        ...
        meddelande: els.meddelande.value.trim(),
    ...
    form.addEventListener("submit", function (e) {
      ...
      var cfg = window.AMPY_MF;
      var payload = collect();
      els.submit.classList.add("is-loading"); els.submit.disabled = true;

      // Same-origin WordPress REST endpoint (backend.php injects window.AMPY_MF).
      // No config → cannot deliver → error state (never fake success).
      if (!cfg || !cfg.endpoint) { showError(); return; }
      ...
      .then(function (d) {
        if (!d || d.ok !== true) throw new Error("not ok");     // strict: only a real 2xx {ok:true}
        showSuccess(payload, true);                             // delivered
      })
      .catch(function () {
        clearTimeout(to);
        showError();                                            // any failure/timeout → error, never fake success
      });
    });
  }

  function boot() {
    var roots = document.querySelectorAll(".ampy-mf");
